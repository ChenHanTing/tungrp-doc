  ...
  class V1MaintainOrdersBody
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SwaggerClient::V1MaintainOrdersBody` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SwaggerClient::V1MaintainOrdersBody`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'product_no')
        self.product_no = attributes[:'product_no']
      end
    ...
    def list_invalid_properties
      ...
      if @product_no.nil?
        invalid_properties.push('invalid value for "product_no", product_no cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @product_no.nil?
    ...
    def hash
      [receiver_name, receiver_phone, brand_id, item, product_no, services, store_id].hash
    end
